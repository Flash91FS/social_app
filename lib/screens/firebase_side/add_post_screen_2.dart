...
const String TAG = "FS - AddPostScreen2 - ";

class AddPostScreen2 extends StatefulWidget {
  final LocationData? loc;

  const AddPostScreen2({Key? key, required this.loc}) : super(key: key);
...
class _AddPostScreen2State extends State<AddPostScreen2> {
  ...
  void postImage(String uid, String username, String profImage, {required bool multiImages}) async {
    // showSnackBar(
    //   context: context,
    ...
    // todo add null check only for selective categories
    // else if (_file == null) {
    ...
    if (!error) {
      ...
      try {
        ...
        if (userLoc == null) {
          ...
          String res = "Error: Something went wrong while trying to upload the post";

          if (multiImages) {
            res = await FireStoreMethods().uploadPostMultiImages(
...
//   }
  ...
  @override
  void didUpdateWidget(covariant AddPostScreen2 oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    int gridItemCont = imgList.length < 4 ? imgList.length + 1 : imgList.length;
    final gridHeight = gridItemCont > 2 ? screenWidth + 20 : screenWidth / 2 + 15;
      ...
      body: SingleChildScrollView(
        child: Column(
          ...
          children: <Widget>[
            ...
            //------------ CHOOSE PICTURE && PICTURE GRID-LIST WIDGET -------------

            _buildGridListWithAddPicWidget(context, screenWidth, darkMode, gridHeight, gridItemCont),

            // _buildChoosePicture(darkMode),

            _buildAddSpotButton(userProvider),
  ...
  Widget _buildChoosePicture(bool darkMode) {
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 7),
  ...
  Widget _buildAddSpotButton(UserProvider userProvider) {
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 7),
      child: SizedBox(
        width: double.infinity,
  ...
  Widget _buildItemPictureForGrid(BuildContext context, double screenWidth, bool darkMode, Uint8List imgFile) {
    return Container(
  ...
  Widget _buildAddPictureButtonForGrid(BuildContext context, double screenWidth, bool darkMode) {
  ...
  Widget _buildGridListWithAddPicWidget(
      BuildContext context, double screenWidth, bool darkMode, double gridHeight, int gridItemCont) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 15, vertical: 3),
      height: gridHeight, //screenWidth + 20,
      child: GridView.builder(
        ...
        itemCount: gridItemCont,
        itemBuilder: (context, index) {
          if (index == gridItemCont - 1 && imgList.length != imagesLimit) {
            return _buildAddPictureButtonForGrid(context, screenWidth, darkMode);
          } else {
            return _buildItemPictureForGrid(context, screenWidth, darkMode, imgList[index]);
          }
        },
      ),
      // child: GridView(
